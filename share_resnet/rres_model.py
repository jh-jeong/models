...
class RResNet(object):
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('unit_1_share'):
        ...
        with tf.variable_scope('unit_2_share'):
        ...
        with tf.variable_scope('unit_3_share'):
    ...
    def _knl_share(self, n_filters):
        """Kernel variables for convolution sharing"""
        n = 3 * 3 * n_filters

        if self.hps.use_bottleneck:
            knl_c1 = tf.get_variable(
                'DW_1', [1, 1, n_filters, n_filters / 4],
                tf.float32, initializer=tf.random_normal_initializer(
                    stddev=np.sqrt(2.0 / n_filters)))
            knl_c2 = tf.get_variable(
                'DW_2', [3, 3, n_filters / 4, n_filters / 4],
                tf.float32, initializer=tf.random_normal_initializer(
                    stddev=np.sqrt(2.0 / n)))
            knl_c3 = tf.get_variable(
                'DW_3', [1, 1, n_filters / 4, n_filters],
                tf.float32, initializer=tf.random_normal_initializer(
                    stddev=np.sqrt(2.0 / n_filters)))
            share = [knl_c1, knl_c2, knl_c3]
        else:
            knl_c1 = tf.get_variable(
                'DW_1', [3, 3, n_filters, n_filters],
                tf.float32, initializer=tf.random_normal_initializer(
                    stddev=np.sqrt(2.0 / n)))
            knl_c2 = tf.get_variable(
                'DW_2', [3, 3, n_filters, n_filters],
                tf.float32, initializer=tf.random_normal_initializer(
                    stddev=np.sqrt(2.0 / n)))
            share = [knl_c1, knl_c2]
    ...
    def _conv_share(self, name, x, kernel, strides):
